...
export default function Notes() {
  ...
  return (
    <div className="flex h-full">
      {/* Notes list */}
      <div
        className={`flex flex-col ${editing ? "w-80 shrink-0 border-r border-gray-200" : "flex-1"}`}
      >
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-4">
          {filtered.length === 0 && (
            <div className="flex flex-col items-center justify-center h-40 text-gray-300">
              ...
              <p className="text-sm">
                {search ? "ไม่พบ note" : "ยังไม่มี note"}
              </p>
            </div>
          )}
          ...
          {pinned.length > 0 && (
            <div>
              ...
              <div
                className={`grid gap-2 ${editing ? "grid-cols-1" : "grid-cols-2 xl:grid-cols-3"}`}
              >
                {pinned.map((m) => (
                  <MemoCard
                    key={m.id}
        ...
        <div className="flex-1 flex flex-col bg-white">
          <div className="flex items-center justify-between px-6 py-3 border-b border-gray-100">
            <span className="text-xs text-gray-400">
              {isNew
                ? "Note ใหม่"
                : `แก้ไขล่าสุด ${formatDate(memos.find((m) => m.id === editing.id)?.updatedAt || new Date())}`}
            </span>
...
function MemoCard({ memo, active, onClick, onPin, onDelete }) {
  return (
    ...
    >
      ...
      <p
        className={`text-sm font-semibold text-gray-800 mb-1 leading-snug ${memo.pinned ? "pr-5" : ""} ${!memo.title && "text-gray-300 font-normal italic"}`}
      >
